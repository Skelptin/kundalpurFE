...
import UpdateDonationType from './UpdateBoli';
...
function BoliHead() {
  ...
  const handlesubmit = async (e) => {
    try {
      ...
      const { data } = await axios.post(`${backendApiUrl}admin/add-boliHead`, {
        boliHead_hi: donationtype_in_hindi,
        boliHead_en: donationtype_in_eng,
      });
      if (data.status === true) {
        setshowloader(false);
        Swal.fire('Great!', 'Boli Head Added Successfully', 'success');
        handleClose();
              ...
              {isData &&
                (rowsPerPage > 0
                  ...
                  >
                    ...
                    <TableCell>{row?.boliHead_hi}</TableCell>
                    <TableCell>{row?.boliHead_en}</TableCell>
                    {/* <TableCell>
