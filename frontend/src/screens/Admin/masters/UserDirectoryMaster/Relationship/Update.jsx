...
function Update({ setOpen, updatedata }) {
  ...
  const handlesubmit = async () => {
    try {
      ...
      const data = {
        relationshipType_hi: commentss,
        relationshipType_en: categoryname,
        id: updatedata?.id,
      };
      serverInstance('admin/edit-relationshipType', 'put', data).then((res) => {
        if (res?.status) {
          setOpen(false);
          setshowloader(false);
  ...
  useEffect(() => {
    if (updatedata) {
      setcategoryname(updatedata?.relationshipType_en);
      setcommentss(updatedata?.relationshipType_hi);
    }
  }, []);
                ...
                <>
                  <div className="inner-input-div2">
                    ...
                    <CustomInput
                      id="commentss"
                      name="commentss"
                      placeholder="Enter Ocassion Type"
                      value={commentss}
                      onChange={(e) => setcommentss(e.target.value)}
                    />
                  </div>
