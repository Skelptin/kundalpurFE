...
const AddCategory = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-occasionType?id=${deleteId}`, 'delete').then(
      (res) => {
        if (res?.status === true) {
          setOpen(false);
                ...
                <>
                  {(rowsPerPage > 0
                    ...
                    >
                      ...
                      <TableCell>{row?.occasionType_hi}</TableCell>
                      <TableCell>{row?.occasionType_en}</TableCell>
