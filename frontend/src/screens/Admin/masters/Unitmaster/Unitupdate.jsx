...
function Unitupdate({ data, handleClose3 }) {
  ...
  const handlesubmit = async () => {
    try {
      ...
      const res = await axios.put(`${backendApiUrl}admin/edit-boliUnit`, {
        id: id,
      ...
      if (res.data.status === true) {
        setshowloader(false);
        Swal.fire('Great!', 'Boli Unit Updated Successfully', 'success');
  ...
  useEffect(() => {
    if (data) {
      setisData(data?.data);
      setdonationtype_in_eng(data.boliUnit_en);
      setdonationtype_in_hindi(data.boliUnit_hi);
      setid(data.id);
    }
  }, []);
  return (
