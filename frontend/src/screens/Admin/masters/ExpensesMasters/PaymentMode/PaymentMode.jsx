...
const PaymentMode = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-paymentMode?id=${deleteId}`, 'delete').then((res) => {
  ...
  const getPaymentMode = () => {
    try {
      serverInstance('admin/get-paymentMode', 'get').then((res) => {
        if (res.status) {
          setisData(res.data)
        }
      })
      ...
      <div>
        <div className="search-header-print">
          ...
          >
            ...
            <Tooltip title="Add Payment Mode">
              <Button
                onClick={() => handleOepn()}
