...
function Update({ setOpen, updatedata }) {
  ...
  const handlesubmit = async () => {
    try {
      setshowloader(true);
      const data = {
        vehicleType_hi: sadsyaTypeHindi,
        vehicleType_en: sadsyaTypeEnglish,
        id:updatedata?.id
      };
      serverInstance('admin/edit-vehicleType', 'put', data).then((res) => {
        if (res?.status) {
          setOpen(false);
          setshowloader(false);
  ...
  useEffect(() => {
    if (updatedata) {
      setSadsyaTypeHindi(updatedata?.vehicleType_hi);
      setSadsyaTypeEnglish(updatedata?.vehicleType_en);
    }
  }, []);
