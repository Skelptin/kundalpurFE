...
const VTMaster = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-vehicleType?id=${deleteId}`, 'delete').then(
      (res) => {
        if (res?.status === true) {
          setOpen(false);
  ...
  const vehicleType = {
    type: ['Light Vehicle', 'Heavy Vehicle'],
  };
  // console.log(vehicleType.type[1])
              ...
              {isData &&
                isData?.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.vehicleType_hi}</TableCell>
                    <TableCell>{item?.vehicleType_en}</TableCell>
                    <TableCell>
                      <Tooltip title="Edit">
                        <img
