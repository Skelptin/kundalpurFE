...
const PaymentType = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-paymentType?id=${deleteId}`, 'delete').then((res) => {
  ...
  const getBankName = () => {
    setloader(true);
    serverInstance('admin/get-paymentType', 'get').then((res) => {
      if (res.data) {
      ...
      <div>
        <div className="search-header-print">
          ...
          >
            ...
            <Tooltip title="Add Payment Type">
              <Button
                onClick={() => handleOepn()}
              ...
              {isData &&
                isData?.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.paymentTypeCode}</TableCell>
