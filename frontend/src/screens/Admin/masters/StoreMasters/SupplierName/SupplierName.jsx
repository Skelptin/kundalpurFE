...
const SupplierName = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-supplierName?id=${deleteId}`, 'delete').then((res) => {
  ...
  const getSupplierDetails = () => {
    setloader(true);
    serverInstance('admin/get-supplierName', 'get').then((res) => {
      if (res.data) {
        setloader(false);
        setisData(res.data);
      }
            ...
            <TableBody>
              {
                isData && isData.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.supplierName_en}</TableCell>
                    <TableCell>{item?.supplierName_hi}</TableCell>
                    <TableCell>
                      <Tooltip title="Edit">
                        <img
