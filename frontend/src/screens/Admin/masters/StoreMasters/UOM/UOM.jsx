...
const UOM = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-uom?id=${deleteId}`, 'delete').then((res) => {
  ...
  const getUOM = () => {
    setloader(true);
    serverInstance('admin/get-uom', 'get').then((res) => {
      if (res.data) {
        setloader(false);
        setisData(res.data);
      }
      ...
      <div>
        <div className="search-header-print">
          ...
          >
            ...
            <Tooltip title="Add Unit Of Measurement">
              <Button
                onClick={() => handleOepn()}
