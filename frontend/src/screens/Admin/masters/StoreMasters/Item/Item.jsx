...
const Item = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-item?id=${deleteId}`, 'delete').then((res) => {
  ...
  const getUOM = () => {
    setloader(true);
    serverInstance('admin/get-item', 'get').then((res) => {
      if (res.data) {
        setloader(false);
        setisData(res.data);
      }
            ...
            <TableBody>
              {
                isData && isData.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.itemNameHindi}</TableCell>
                    <TableCell>{item?.itemNameEnglish}</TableCell>
