...
const EmpStatus = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-employeeStatus?id=${deleteId}`, 'delete').then((res) => {
      ...
      >
        <Fade in={open1}>
          <Box sx={style}>
            ...
            <UpdateStatus setOpen={handleClose1} updatedata={updatedata}/>
          </Box>
        </Fade>
      </Modal>
      <div>
          ...
          {isData &&
                isData?.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.employeeStatus_code}</TableCell>
