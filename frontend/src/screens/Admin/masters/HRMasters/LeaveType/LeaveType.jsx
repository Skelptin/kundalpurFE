...
const EmpType = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-leaveType?id=${deleteId}`, 'delete').then((res) => {
           ...
           {isData &&
                isData?.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.leaveType_code}</TableCell>
                    <TableCell>{item?.leaveType_hi}</TableCell>
                    <TableCell>{item?.leaveType_en}</TableCell>
