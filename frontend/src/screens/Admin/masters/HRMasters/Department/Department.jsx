...
const Department = ({ setopendashboard }) => {
  ...
  const handleClose4 = () => {
    setOpen3(false);
    serverInstance(`admin/delete-department?id=${deleteId}`, 'delete').then((res) => {
      ...
      >
        <Fade in={open1}>
          <Box sx={style}>
            ...
            <UpdateDept  setOpen={handleClose1} updatedata={updatedata}/>
          </Box>
        </Fade>
      </Modal>
      <div>
        <div className="search-header-print">
          ...
          >
            ...
            <Tooltip title="Add Department Name">
              <Button
                onClick={() => handleOepn()}
            ...
            {isData &&
                isData?.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{item?.departmentName_hi}</TableCell>
                    <TableCell>{item?.departmentName_en}</TableCell>
