...
const UpdateFamiliy = ({
  ...
  const addFamilyDirectory = () => {
    ...
    try {
      ...
      serverInstance('committee/edit-family', 'put', data).then((res) => {
        if (res?.status) {
          handleClose();
          setshowloader(false);
  ...
  useEffect(() => {
    ...
    if (updateData) {
      ...
      setDonationItems(updateData?.familyMembers);
      setage(updateData?.Age);
      setFullName(updateData?.Name);
      setAadharNo(updateData?.AadharNo);
      setpanNo(updateData?.PanNo);
      setEmail(updateData?.Email);
      setDOA(
        new Date(updateData?.DateOfOccasion).toISOString().substring(0, 10),
      );
      setDOB(new Date(updateData?.DateOfBirth).toISOString().substring(0, 10));
      setRemark(updateData?.Remark);
      setsadsyaname(updateData?.SadsyaType);
      setOcassionname(updateData?.OccasionType);
    }
    setrole(Number(sessionStorage.getItem('userrole')));
  }, []);

  console.log('data from data is data is ', donationItems);
