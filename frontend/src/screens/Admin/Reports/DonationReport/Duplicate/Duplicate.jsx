...
const Duplicate = ({ setopendashboard }) => {
  ...
  const getall_donation = () => {
    ...
    setvoucherto('');
    setsearchvalue('');
    serverInstance('admin/checkDoubleReceipt-donation', 'get').then((res) => {
      if (res.status) {
    ...
    try {
      if (searchvalue) {
        ...
        const res = await axios.get(
          `${backendApiUrl}admin/checkDoubleReceipt-donation?search=${searchvalue}&type=${2}`,
        );
        ...
        serverInstance(
          `admin/checkDoubleReceipt-donation?fromDate=${datefrom}&toDate=${dateto}&fromVoucher=${voucherfrom}&toVoucher=${voucherto}&modeOfDonation=${2}`,
          'post',
