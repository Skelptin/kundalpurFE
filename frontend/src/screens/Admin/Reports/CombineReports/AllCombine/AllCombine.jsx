...
import ElectronicTotal from '../../../compoments/AllComBineTotal';
...
import PrintContent from '../../../compoments/PrintAllCombine';
import ModalElecReceipt from '../../../Reciept/AllCombineReceipt';
...
const DonationCombine = ({ setopendashboard }) => {
  ...
  const getall_donation = () => {
    ...
    setvoucherto('');
    setsearchvalue('');
    serverInstance('admin/combine-elecmanualDonation', 'post').then((res) => {
      if (res.status) {
  ...
  const printreceipt = (row) => {
    if (row.active === '0') {
      ...
      navigation('/admin-panel/room/AllCombineReceipt', {
        state: {
          userdata: row,
        },
      });
  ...
  const ExportToExcel = () => {
    ...
    isData.map((item, index) => {
      data.push({
        ...
        'Head/Item': item?.details.map((row) => {
          return row.type;
        }),
  ...
  const filterdata = async (e) => {
    ...
    try {
      if (searchvalue) {
        ...
        // if (typeofdonation === 0) {
          serverInstance(
            `admin/combine-elecmanualDonation?fromDate=${datefrom}&toDate=${dateto}&modeOfDonation=${typeofdonation}&fromVoucher=${voucherfrom}&toVoucher=${voucherto}`,
            'post',
        ...
        //   serverInstance(`admin/combine-elecmanualDonation?fromDate=$(datefrom}&toDate=${dateto}&modeOfDonation=${typeofdonation}&fromVoucher=${voucherfrom}&toVoucher=${voucherto}`,post).then((res)=>{
        //     if (res.status) {
        //       let filterData = res?.data.filter(
        //         (item) => item?.isActive === true,
        //       );
  ...
  console.log("All Donation Data Is ",isData);
  useEffect(() => {
    getallemp_list();
    getall_donation();
  ...
  useEffect(() => {
    var filtered = isDataDummy?.filter(
      (dt) =>
        ...
        dt?.created_by?.toLowerCase()?.indexOf(userType) > -1 &&
    ...
    if (type) {
      filtered = filtered?.map((item) => {
        if (item?.details?.find((typ) => typ.type == type)) {
          return item;
        } else {
          return;
        }
    ...
    if (remark) {
      filtered = filtered?.map((item) => {
        if (item?.details?.find((typ) => typ.remark == remark)) {
          return item;
        } else {
          return;
        }
            ...
            <TableHead style={{ background: '#FFEEE0' }}>
              <TableRow>
                ...
                <TableCell>
                  ...
                  <i
                    style={{ marginLeft: '0.5rem' }}
                    onClick={() => sortDataAmount('details')}
                    class={`fa fa-sort`}
                  />
                </TableCell>
                <TableCell>
