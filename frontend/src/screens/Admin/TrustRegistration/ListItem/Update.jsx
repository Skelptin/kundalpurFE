...
const Update = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      serverInstance('trust/edit-trust', 'put', data).then((res) => {
        if (res.status) {
          handleClose()
          setshowloader(false)
  ...
  useEffect(() => {
    ...
    getBankName()
    if (updatedata) {
      setTrustCode(updatedata?.TrustCode)
      setTrustName(updatedata?.TrustName)
      setMainTrustName(updatedata?.MainTrustName)
      setTrustType(updatedata?.TrustType)
      setMobileNo(updatedata?.MobileNo)
      setEmail(updatedata?.Email)
      setLocation(updatedata?.Location)
      setAddress(updatedata?.TrustAddress)
      setState(updatedata?.State)
      setCity(updatedata?.City)
      setPincode(updatedata?.PinCode)
      setBankName(updatedata?.NameOfBank)
      setAccountDetails(updatedata?.AccountDetails)
      setIFSCCode(updatedata?.IFSC_Code)
      setPanNo(updatedata?.PAN_Number)
      setGST(updatedata?.GST)
      ...
      <ThemeProvider theme={theme}>
        <form onSubmit={handleSubmit}>
          ...
          <Grid item xs={12} md={2}>
            ...
            <Grid item xs={12} md={4}>
              <CustomInputLabel htmlFor="PANNo">
                PAN Number
              </CustomInputLabel>
              <CustomInput
                required
