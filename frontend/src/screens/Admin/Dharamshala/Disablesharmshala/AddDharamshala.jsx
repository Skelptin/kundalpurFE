...
const AddDharamshala = ({ setopendashboard }) => {
  ...
  const getall_donation = () => {
    setloader(true);
    serverInstance('admin/get-disableDharamshala', 'get').then((res) => {
      console.log('res data get disabled dharamshala', res);
      if (res?.data) {
        setloader(false);
        setisData(res?.data);
      }
  ...
  console.log('data in state',isData);
                ...
                <>
                  {(rowsPerPage > 0
                    ...
                    >
                      ...
                      <TableCell>{row?.dharamshalaName}</TableCell>
                      <TableCell>{row?.dharamshalaDesc?.slice(0, 40)}</TableCell>
