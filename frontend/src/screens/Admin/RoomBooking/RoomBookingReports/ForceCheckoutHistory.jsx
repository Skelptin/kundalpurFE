...
const ForceCheckoutHistory = ({ setopendashboard }) => {
  ...
  const getall_donation = () => {
    ...
    serverInstance('room/forceCheckoutHistory', 'get').then((res) => {
      console.log(res);
  ...
  const downloadcheckout = (row) => {
    navigation('/admin-panel/Forcheckouthistory', {
      state: {
        data: row,
      },
    });
  ...
  const filterdata = async (e) => {
    ...
    try {
      serverInstance(
        `room/forceCheckoutHistory?fromDate=${datefrom}&toDate=${dateto}`,
        'get',
      ).then((res) => {
        if (res?.data) {
  ...
  const onSearchByOther = (e, type) => {
    ...
    if (type === 'checkoutByName') {
      setcheckoutBy(e.target.value.toLowerCase());
    }
    if (type == 'coutDate') {
...
export default ForceCheckoutHistory;
