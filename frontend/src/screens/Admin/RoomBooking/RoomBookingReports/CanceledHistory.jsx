...
const CanceledHistory = ({ setopendashboard }) => {
  ...
  const getall_donation = () => {
    ...
    serverInstance('room/cancelHistory', 'get').then((res) => {
      if (res.data) {
        setloader(false);
  ...
  const ExportToExcel = () => {
    ...
    var data = [];
    // dharmasalaData?.name roomAmountSum advanceAmountSum  roomNumbers?.map((item) => (
    //   <span>{item},</span>
    // ))
  ...
  const downloadrecept = (row) => {
    navigation('/admin-panel/room/CancelOnlyreciept', {
      state: {
        data: row,
      },
    });
  ...
  const filterdata = async (e) => {
    ...
    try {
      serverInstance(
        `room/cancelHistory?fromDate=${datefrom}&toDate=${dateto}`,
        'get',
      ).then((res) => {
        if (res?.data) {
          setisData(res?.data);
        ...
        <Fade in={open1}>
          <Box sx={style}>
            <div>
              ...
              <Printcancel isData={isData} setOpen1={setOpen1} />;
            </div>
          </Box>
        </Fade>
      </Modal>
