...
const UpdatePR = ({ updatedata, openEdit, closeEdit }) => {
    ...
    const handleUpdate = (e) => {
        ...
        try {
            ...
            serverInstance('store/edit-purchaseRequisition ', 'put', data).then((res) => {
                if (res.status) {
                    closeEdit()
                    setStep(1)
    ...
    useEffect(() => {
        ...
        getSupplier();
        if (updatedata) {
            setPurchaseReq(updatedata?.purchaseRequisitionNo)
            setSupCode(updatedata?.supplierCode)
            setSupName(updatedata?.supplierName)
            setDeptCode(updatedata?.departmentCode)
            setDeptName(updatedata?.departmentName)
            setAddress(updatedata?.address)
            setCity(updatedata?.city)
            setState(updatedata?.state)
            setPincode(updatedata?.pincode)
            setDeliveryDate(updatedata?.deliveryDate)
            setApprover1(updatedata?.approver1)
            setApprover2(updatedata?.approver2)
            setApprover3(updatedata?.approver3)
            setApprover4(updatedata?.approver4)
            setContactNo(updatedata?.mobileNo)
            setRemark(updatedata?.remark)
            setdbItems(updatedata?.purchaseRequisitionLists)
            console.log('UD', updatedata?.purchaseRequisitionLists)
