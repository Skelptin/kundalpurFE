...
const VehiclePass = ({ setopendashboard }) => {
  ...
  const filterdata = async (e) => {
    ...
    try {
      if (searchvalue) {
        serverInstance(
          `vehicle/search-vehicle?search=${searchvalue}`,
          'get',
        ).then((res) => {
          if (res?.status) {
            setemplist(res?.data);
        ...
        serverInstance(
          `vehicle/get-vehicle?fromDate=${datefrom}&toDate=${dateto}`,
          'get',
        ).then((res) => {
          if (res?.status) {
            setemplist(res?.data);
  ...
  const getallemp_list = () => {
    serverInstance('vehicle/get-vehicle', 'get').then((res) => {
      if (res.status) {
        setemplist(res.data);
      } else {
        Swal('Error', 'somthing went  wrong', 'error');
  ...
  console.log('getting data is ', emplist);
