...
const GateOut1 = ({ setopendashboard }) => {
  let filterData;
  ...
  const getallemp_list = () => {
    serverInstance('vehicle/gateout-history', 'get').then((res) => {
      if (res.status) {
        setemplist(res.data);
      } else {
  ...
  const filterdata = async (e) => {
    ...
    try {
      if (searchvalue) {
        serverInstance(
          `vehicle/search-gateoutHistory?search=${searchvalue}`,
          'get',
        ).then((res) => {
          if (res?.status) {
            setemplist(res?.data);
        ...
        serverInstance(
          `vehicle/gateout-history?fromDate=${datefrom}&toDate=${dateto}`,
          'get',
        ).then((res) => {
          if (res?.status) {
            setemplist(res?.data);
