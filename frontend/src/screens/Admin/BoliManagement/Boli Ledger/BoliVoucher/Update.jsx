...
const Update = ({
  ...
  const addCashDonation = async (e) => {
    ...
    try {
      ...
      serverInstance('boli/edit-boliVoucher', 'put', data).then((res) => {
        if (res?.status) {
          handleClose();
          setshowloader(false);
  ...
  useEffect(() => {
    // getall_donatiions();
    if (updateData) {
      setDonationDate(updateData?.Date);
      setDonationTime(updateData?.donationTime);
      setledgerno(updateData?.LedgerNo);
      setphonono(updateData?.MobileNo);
      ...
      setboliamount(updateData?.BoliAmount);
      setDonationItems(updateData?.boliVoucherList);
    }
    setrole(Number(sessionStorage.getItem('userrole')));
  }, []);

  console.log('update data from vouchetrr', updateData);
  return (
    <Box>
