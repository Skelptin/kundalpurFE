...
import AddboliVoucher from './AddBoliVoucher';
...
const BoliLedger = ({ setopendashboard }) => {
  ...
  const getall_donation = () => {
    ...
    setvoucherto('');
    setsearchvalue('');
    serverInstance('boli/get-boliVoucher', 'get').then((res) => {
      if (res.status) {
        setloader(false);
        setisData(res?.data);
    ...
    try {
      if (searchvalue) {
        ...
        const res = await axios.get(
          `${backendApiUrl}boli/search-boliVoucher?search=${searchvalue}`,
        );
        ...
        serverInstance(
          `boli/get-boliVoucher?fromDate=${datefrom}&toDate=${dateto}`,
          'get',
        ).then((res) => {
          if (res.status) {
  ...
  const handleClose2 = () => {
    setOpen2(false);
    serverInstance(`boli/delete-boliVoucher?id=${deleteId}`, 'delete').then(
      (res) => {
        if (res.status === true) {
        ...
        <div style={{ marginLeft: '3rem' }}>
          ...
          <div className="table-div-maain">
            ...
            >
              ...
              <TableBody>
                <TableRow onClick={() => alert('clicked')}></TableRow>
                  ...
                  <>
                    {(rowsPerPage > 0
                      ...
                      >
                        ...
                        <TableCell>{row?.BoliAmount}</TableCell>
                        <TableCell>
                          <Tooltip title="Print Certificate">
                            <img
                              style={{ width: '20px', marginLeft: '0.2rem' }}
                              onClick={() => handleClickOpen1(row?.Boli_id)}
                              src={Delete}
                              alt=" Print"
                            />
                          </Tooltip>
