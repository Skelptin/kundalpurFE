...
const Addboli = ({
  ...
  // const no = async () => {
  //   const alreadyExists = await serverInstance('boli/get-boliLedger', 'get')
  ...
  const addCashDonation = async () => {
    ...
    const mobileExists = alreadyExists.data.some(item => item.MobileNo === mobileNo);

    if (mobileExists) {
      handleClose();
      setshowloader(false);
      Swal.fire('Error!', 'Mobile number already exists. Please enter a unique mobile number.', 'error');
      ...
      try {
        ...
        serverInstance('boli/add-boliLedger', 'post', data).then((res) => {
          if (res?.status) {
            handleClose();
            setshowloader(false);
