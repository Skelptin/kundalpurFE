...
const UpdateBoli = ({
  ...
  console.log('boli units and heads', donationItems);
  ...
  const addCashDonation = async (e) => {
    ...
    if (fullName) {
      const res = await axios.put(`${backendApiUrl}boli/edit-boli`, {
      ...
      console.log('update boli res', res?.data?.status);
  ...
  useEffect(() => {
    if (updatedata) {
      ...
      setstatus(updatedata?.BoliStatus);
      setDonationItems(updatedata?.boliItem);
    }
