...
const UnderlinedTab = ({ tabs, themeColor, handleClose }) => {
  ...
  function getTabColor(val) {
    if (val === 0) return themeColor?.received || defaultColor;
    if (val === 1) return themeColor?.payment || defaultColor;
    if (val === 2) return themeColor?.journal || defaultColor;
    return themeColor?.contra || defaultColor;
  }
  return (
    <div>
      <Box
