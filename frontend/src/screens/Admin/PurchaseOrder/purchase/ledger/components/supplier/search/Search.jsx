...
import { fetchLedgerSupplierList, fetchLedgerSupSearchList, ledgerSelector } from "../../../../../reducers/ledger/LedgerSlice";
export default function Search() {
	...
	const supplierArr = [{ value: "0", label: "Select Party Name" }];
	{
		pv &&
			pv.ledgerSupplierList && pv.ledgerSupplierList.length>0 &&
			pv.ledgerSupplierList.map((pv, index) => {
				codeArr.push({
					value: pv.id,
					label: pv.pv_code
				});
	...
	const searchAction = () => {
		...
		if (supplierValue != 0) {
			search_str += "&purchase_order__supplier__supplier_name__icontains=" + supplierValue;
		}

		dispatch(
			fetchLedgerSupSearchList({
				offset: 0,
				limit: 15,
