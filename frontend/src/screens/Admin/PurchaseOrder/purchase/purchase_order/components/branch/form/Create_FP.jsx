...
export default function Create_FP() {
  ...
  const [selectedFp, setSelectedIFp] = useState([{}]);
  ...
  /* Searching selected branch */
  const searchSupplier = async (supplierId) => {
    setPlaceHolderLoader("Loading...");
    if (supplierId > 0) {
      // const ss = branch.supplierList.find(
      // 	(branch) => branch.id === parseInt(supplierId),
      // );
  ...
  const create = (data) => {
    ...
    if (data.order_list.find((x) => x.item_master == null)) {
      ...
      dispatch(createPurchaseBranchOrder({ data, toast })).then((response) => {
