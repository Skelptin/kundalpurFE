...
import { Update_Payment_Branch } from "../form/Update_Payment_Branch";
export default function Payment_list_Bra() {
	const dispatch = useDispatch(); 
	const allpv = useSelector(pvSelector);
	const childPvRef = useRef();
	const [pvData, setPvData] = useState(null);
	const handleShowModal = () => {
		childPvRef.current.showPvBranch();
	}
	const selectedRow = (e) => {
		...
		<>
			...
			<div className="wrapper_abc">
				<table>
					...
					{allpv &&
							allpv.PvBranchList &&
							allpv.PvBranchList.results &&
							allpv.PvBranchList.results.map((pv, index) => (
								<tr>
									<td>
										<input
											type="checkbox"
									...
									<td>{pv?.purchase_order?.branch?.branch_name}</td>
				...
				{pvData && <Update_Payment_Branch pv={pvData} ref={childPvRef} />}
